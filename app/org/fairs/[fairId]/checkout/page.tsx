...
import { CheckoutClient } from "@/app/admin/fairs/[fairId]/checkout/CheckoutClient";
import { Card } from "@/components/ui";
import { isPromotionInWindow, type ActivePromotion } from "@/lib/promotions";

// Mirrors app/admin/fairs/[fairId]/checkout/page.tsx exactly — same
// CheckoutClient, same Server Actions (migration 0046/requireFairStaff()
// let org staff call them for a fair their own org owns) — only the
// page-level gate and the explicit org-ownership check differ from the
// admin version, which relies on /admin's own requireAdmin() layout gate
// instead.
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {fair.allow_in_person && !fair.stripe_terminal_location_id && (
        <Card className="max-w-lg border border-amber-200 bg-amber-50">
          <p className="text-sm text-amber-800">
            No Terminal reader set up for this fair yet — ask a platform admin to set one up
      ...
      {!fair.allow_in_person && !fair.allow_wallet && !fair.allow_cash && (
        <Card className="max-w-lg border border-amber-200 bg-amber-50">
          <p className="text-sm text-amber-800">
            No payment options are enabled for this fair — nothing to charge here.
          </p>
        </Card>
      )}
