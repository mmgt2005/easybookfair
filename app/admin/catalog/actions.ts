...
export async function createCatalogItem(formData: FormData) {
  ...
  const { error } = await supabase.from("catalog_items").insert({
    ...
    author_phone: String(formData.get("author_phone") ?? "").trim() || null,
...
export async function updateCatalogItem(catalogItemId: string, formData: FormData) {
  const supabase = await createClient();
  ...
  // Only touch image_url if a new file was actually chosen — otherwise
  // leave the existing image alone rather than clearing it.
  const imageFile = formData.get("image") as File | null;
  if (imageFile && imageFile.size > 0) {
    const extension = imageFile.name.split(".").pop() || "jpg";
    const path = `${crypto.randomUUID()}.${extension}`;
    ...
    update.image_url = supabase.storage.from("catalog-images").getPublicUrl(path).data.publicUrl;
  }

  const { error } = await supabase.from("catalog_items").update(update).eq("id", catalogItemId);
